...
### Finding 7: Confidence Matrices Reveal Consistent Patterns Across All Runs

**We computed 2×2 confidence matrices showing average confidence for each confusion matrix category across all 4 runs:**

#### Run 1: ToxicChat + Heuristic

| | Refusal (Pred) | Compliance (Pred) |
|---|---|---|
| **Toxic (Actual)** | TP: **0.682** | FN: **0.588** |
| **Safe (Actual)** | FP: 0.510 | TN: 0.541 |

- **Key Pattern**: TP confidence (0.682) is **16% higher** than FN confidence (0.588)
- When the model correctly refuses toxic prompts, it does so with higher confidence
- False negatives show moderate confidence, not distinctively low

#### Run 2: WildGuardMix + Heuristic

| | Refusal (Pred) | Compliance (Pred) |
|---|---|---|
| **Toxic (Actual)** | TP: **0.650** | FN: **0.599** |
| **Safe (Actual)** | FP: 0.648 | TN: 0.569 |

- **Key Pattern**: TP confidence (0.650) is **8.5% higher** than FN confidence (0.599)
- FP and TP have nearly identical confidence (0.648 vs 0.650) - the heuristic cannot distinguish
- This explains the heuristic's 41.2% accuracy on adversarial data

#### Run 3: ToxicChat + WildGuard

| | Refusal (Pred) | Compliance (Pred) |
|---|---|---|
| **Toxic (Actual)** | TP: **0.679** | FN: **0.617** |
| **Safe (Actual)** | FP: 0.509 | TN: 0.553 |

- **Key Pattern**: TP confidence (0.679) is **10% higher** than FN confidence (0.617)
- Similar pattern to Run 1, suggesting ToxicChat dataset characteristics drive this
- WildGuard classifier doesn't significantly change confidence patterns vs heuristic

#### Run 4: WildGuardMix + WildGuard

| | Refusal (Pred) | Compliance (Pred) |
|---|---|---|
| **Toxic (Actual)** | TP: **0.663** | FN: **0.607** |
| **Safe (Actual)** | FP: 0.675 | TN: 0.607 |

- **Key Pattern**: TP confidence (0.663) is **9.2% higher** than FN confidence (0.607)
- **Critical observation**: FN and TN have **identical** confidence (0.607)
- This confirms we cannot use confidence alone to identify false negatives

#### Cross-Run Insights

**1. True Positives Always Show Higher Confidence:**
- Across all 4 runs, TP confidence > FN confidence by 8.5-16%
- When the model correctly refuses toxic content, it's more confident

**2. Dataset Effect:**
- ToxicChat runs (1 & 3): Larger TP-FN gap (16% and 10%)
- WildGuardMix runs (2 & 4): Smaller TP-FN gap (8.5% and 9.2%)
- Adversarial prompts (WildGuardMix) create more ambiguous confidence signals

**3. Classifier Effect is Minimal:**
- Heuristic vs WildGuard shows similar confidence patterns
- The difference is in **classification accuracy** (41% vs 56%), not confidence calibration

**4. The FN-TN Problem:**
- In all runs, FN confidence ≈ TN confidence (within 3%)
- **Implication**: Filtering low-confidence compliance would remove safe responses, not just false negatives

**Why This Matters:**

> [!IMPORTANT]
> These matrices confirm that while confidence correlates with **correct refusals** (TP > FN), it **cannot** be used to identify false negatives without also filtering out legitimate safe responses. The confidence score is a signal of model certainty, not classification correctness.
